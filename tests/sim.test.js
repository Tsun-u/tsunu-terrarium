...
// ---- 新家具批次（2026-07-13）：sofa/bunk/cradle/campfire/gathering，走既有 SHOP.decor 通用機制 ----

test('buyDecor：五款新家具皆能依 SHOP.decor 價格購買、正確記入 decor 與 ownedDecor', () => {
  const newKinds = ['sofa', 'bunk', 'cradle', 'campfire', 'gathering'];
  const world = Sim.newWorld(rngFactory(60));
  world.hearts = 1000;

  newKinds.forEach((kind, i) => {
    const res = Sim.buyDecor(world, kind, 10 * i, 20 * i);
    assert.strictEqual(res.ok, true, `${kind} 應購買成功`);
    assert.strictEqual(res.price, C.SHOP.decor[kind], `${kind} 應照 SHOP.decor 價格扣款`);
    assert.deepStrictEqual(world.decor[i], { kind, x: 10 * i, y: 20 * i });
  });
  assert.deepStrictEqual(world.ownedDecor, newKinds);
});

test('buyDecor：五款新家具存讀 roundtrip 後 kind/座標不失真', () => {
  let store = {};
  global.localStorage = {
    getItem: (k) => (Object.prototype.hasOwnProperty.call(store, k) ? store[k] : null),
    setItem: (k, v) => { store[k] = String(v); },
  ...
  try {
    const newKinds = ['sofa', 'bunk', 'cradle', 'campfire', 'gathering'];
    const world = Sim.newWorld(rngFactory(61));
    world.hearts = 1000;
    newKinds.forEach((kind, i) => Sim.buyDecor(world, kind, 100 + i, 50 + i));
    Sim.save(world);

    const loaded = Sim.load();
    assert.ok(loaded, 'load 應回傳非 null');
    assert.strictEqual(loaded.world.decor.length, newKinds.length);
    newKinds.forEach((kind, i) => {
      assert.deepStrictEqual(loaded.world.decor[i], { kind, x: 100 + i, y: 50 + i });
    });
    assert.deepStrictEqual(loaded.world.ownedDecor, newKinds);
  } finally {
    delete global.localStorage;
  }
});
