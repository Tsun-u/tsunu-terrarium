...
(() => {
  ...
  function decorSprite(kind, tMs) {
    ...
    if (kind === 'flower') {
      ...
      // 床身寬度 18→22：純視覺比例調整（原本偏瘦高），貝殼主體等比例加寬，木柱／梯柱同步右移
      cv.width = 22; cv.height = 34;
      px(s, '#8a5f3a', [[2, 3, 2, 28], [18, 3, 2, 28]]);   // 木柱拉長，貫穿整個床身
      const shell = (y, c1, c2) => {
        px(s, c1, [[3, y + 2, 16, 3], [4, y + 1, 14, 1], [7, y, 8, 1]]);
        px(s, c2, [[6, y + 1, 1, 3], [10, y + 1, 1, 4], [14, y + 1, 1, 3]]);
      };
      shell(3, '#ffd3e0', '#e8aabb');    // 上舖（粉貝）
      shell(26, '#d3f0ff', '#a9cfe0');   // 下舖（藍貝）— 原 11，下移拉開間距
      px(s, '#fff3b8', [[4, 4, 3, 1], [4, 27, 3, 1]]);     // 小枕頭：下舖枕頭同步下移
      px(s, '#c79868', [[20, 8, 1, 23]]);                  // 梯柱拉長配合床身增高
      px(s, '#c79868', [[19, 11, 3, 1], [19, 17, 3, 1], [19, 23, 3, 1], [19, 29, 3, 1]]); // 梯橫檔：3階→4階
